...
@protocol MTGLRenderer
- (BOOL) isValid;
- (NSString *) fragmentShader;
- (void) resolveUniforms: (GLuint) program;
- (void) setFrame: (MTVideoFrame *) frame;
- (BOOL) prepareRender;
@end

@interface MTGLRenderer_RGB : NSObject <MTGLRenderer>
{
...
@implementation MTGLRenderer_RGB
...
//把frame转化成纹理
- (void) setFram:(MTVideoFrame *)frame{
    MTVideoFrameRGB *rgbFrame = (MTVideoFrameRGB *)frame;
...
- (BOOL) prepareRender
{
    ...
    //给shader里的变量赋值
    /*
     第一个参数指定被赋值变量的位置，同一个shader里面会定义多个输出对象，每个都有自己的输出位置，可以这样获得:
     _uniformSampler = glGetUniformLocation(program,"s_texture");
     这就是取得uniform变量s_texture的位置，赋值给——uniformSampler。下面方法第二个参数就是指定哪个纹理，传入n，就是GL_TEXTURE0+n 槽位的纹理，这里传入0，结合代码3.1和3.2， 其实就是把_texture。然后_uniformSampler是s_texture的位置，所以整体就是把_texture赋值给了shader里面的s_texture变量
...
@interface MTGLRenderer_YUV : NSObject<MTGLRenderer>
{
...
@implementation MTGLRenderer_YUV
...
- (void) setFrame:(MTVideoFrame *)frame
{
    MTVideoFrameYUV *yuvFrame = (MTVideoFrameYUV *)frame;
             ...
             decoder: (MTDecoder *) decoder
{
    ...
    if (self) {
        ...
        if ([decoder setupVideoFrameFormat:MTVideoFrameFormatYUV]) {
            
            _renderer = [[MTGLRenderer_YUV alloc] init];
            
        } else {
            
            _renderer = [[MTGLRenderer_RGB alloc] init];
        }
...
- (void) render:(MTVideoFrame *)frame{
    ...
    //OpenGL的绘制结果不是直接显示到屏幕上，而是存了起来， 这个存储的东西就是FBO， 所以FBO里包含了color, depth, stencil等一些用于显示的信息
    ...
    //OpenGL把数据输入给我们绑定的FBO， FBO管理着各种buffer，其中就有color buffer。 _renderbuffer就是绑定在当前FBO上的color buffer，存储着颜色信息， 第一步glBindRenderbuffer指定下面对GL_RENDERBUFFER的操作是使用GL——RENDERBUFFER，然后_context显示render buffer。然后数据就被显示到屏幕上了
    glBindRenderbuffer(GL_RENDERBUFFER, _renderbuffer);
